...
Renderer.prototype.insertCircleVertices = function(transform, radius, x, y, r, g, b) {
  ...
  for (var i = 0; i < this.circleResolution; i++) {
    var i4 = i * 4;
    var v1 = new liquidfun.b2Vec2(vertices[i4] * radius + x, vertices[i4 + 1] * radius + y);
    var v2 = new liquidfun.b2Vec2(vertices[i4 + 2] * radius + x, vertices[i4 + 3] * radius + y);

    liquidfun.b2Vec2.Mul(v1, transform, v1);
    liquidfun.b2Vec2.Mul(v2, transform, v2);
...
Renderer.prototype.transformAndInsert = function(v1, v2, transform, r, g, b) {
  var transformedV1 = new liquidfun.b2Vec2(),
    transformedV2 = new liquidfun.b2Vec2();

  liquidfun.b2Vec2.Mul(transformedV1, transform, v1);
  liquidfun.b2Vec2.Mul(transformedV2, transform, v2);
...
liquidfun.b2CircleShape.prototype.draw = function(transform) {
  var circlePosition = this.position,
    center = new liquidfun.b2Vec2(circlePosition.x, circlePosition.y);
 // liquidfun.b2Vec2.Mul(center, transform, center);
  renderer.insertCircleVertices(transform, this.radius, center.x, center.y, 0, 0, 0, 5);
};

liquidfun.b2ChainShape.prototype.draw = function(transform) {
  renderer.transformVerticesAndInsert(this.vertices, transform, 0, 0, 0);
};


liquidfun.b2EdgeShape.prototype.draw = function(transform) {
  renderer.transformAndInsert(this.vertex1, this.vertex2, transform, 0, 0, 0);
};

liquidfun.b2PolygonShape.prototype.draw = function(transform) {
...
function drawParticleSystem(system) {
  ...
  var maxParticles = particles.length,
    transform = new liquidfun.b2Transform();
